...
- *Thinking blocks* — **not rendered, and no toggle.** Verified: thinking content is never persisted. All 3,968 thinking blocks in the corpus (re-measured 2026-07-20; the corpus grows, so treat the exact figure as indicative, not exact) carry `"thinking": ""` with only an encrypted `signature`; the reasoning text is not written to the transcript. A toggle would reveal nothing, so it is dead UI and is cut. The parser already skips empty thinking blocks. If Claude Code ever begins persisting thinking text, this is a small additive change.
